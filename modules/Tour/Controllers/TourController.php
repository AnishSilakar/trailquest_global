...
if(!empty($request->query('cat_id')))
{ 
    $pagecategory=New TourCategory();
    $selected_category= $pagecategory->where('id',$request->query('cat_id'))->orderBy('id','desc')->first();
    if(!empty($selected_category->content && $selected_category->image_id)){
  $page_category = [$selected_category->name,$selected_category->content,$selected_category->gallery,$selected_category->image_id,$selected_category->tourheading];
}

 $all_category = $this->tourCategoryClass::where('parent_id',$request->query('cat_id'))->orderBy('id', 'desc')->where('status', 'publish')
            ->with(['translation'])
            ->get()
            ->toTree();
            ...
            // Get all tours under this category
            $tours = Tour::where('category_id', $category->id)->paginate($limit);
            
            // Get translated name (fallback to default if not available) 
    $category_name = $category->name;
    $category_description = $category->content ?? ''; // Or whatever field contains the description
