...
window.uploaderModal = new Vue({
    ...
    data:{
        ...
        folders:[],
        breadcrumbs:[],
        accept_type:sarbatraCMSUrl.media.groups.default.mime.join(',')
    },
    mounted(){
        let me = this;
    ...
    methods:{
        ...
        reloadFolder:function(){
            var me = this;
            $.ajax({
                url:sarbatraCMSUrl.url+'/media/folder',
                type:'get',
                data:{
                    parent_id:this.currentFolder.id,
                },
        ...
        show(configs){
            ...
            this.accept_type = sarbatraCMSUrl.media.groups[configs.file_type]['mime']
        },
        hide(){
        ...
        removeFiles() {
            var me = this;
            sarbatraCMSUrlApp.showConfirm({
                message: i18n.confirm_delete,
                callback: function(result){
                    if(result){
                        me.isLoading = true;
                        $.ajax({
                            url:sarbatraCMSUrl.admin_url+'/module/media/removeFiles',
                            type:'POST',
                            data:{
                            ...
                            success:function (data) {
                                if(data.status === 1){
                                    //sarbatraCMSUrlApp.showSuccess(data);
                                }
                                if(data.status === 0){
                                    sarbatraCMSUrlApp.showError(data);
                                }
                                me.isLoading = false;
                                me.reloadLists();
                            },
        ...
        reloadLists(){
            ...
            me.isLoading = true;
            $.ajax({
                url:sarbatraCMSUrl.admin_url+'/module/media/getLists',
                type:'POST',
                data:{
        ...
        upload(files){
            ...
            for(var i = 0; i < files.length ; i++){
                ...
                me.isLoading = true;
                $.ajax({
                    url:sarbatraCMSUrl.admin_url+'/module/media/store',
                    data:d,
                    dataType:'json',
                    type:'post',
                    contentType: false,
        ...
        methods:{
            deleteFolder:function (){
              ...
              if(!c) return;
                ...
                this.saving = true;
                $.ajax({
                    url:sarbatraCMSUrl.url+'/media/folder/delete',
                    data:{
                        id:this.folder.id,
                    },
                    type:'post',
            ...
            saveName:function(){
                ...
                this.saving = true;
                $.ajax({
                    url:sarbatraCMSUrl.url+'/media/folder/store',
                    data:{
                        id:this.folder.id,
