...
function sectionSliderHalf() {
  const sectionSlider = document.querySelectorAll('.home-section-slider');
  if (!sectionSlider.length) return;
  ...
  for (let i = 0; i < sectionSlider.length; i++) {
    ...
    if (el.getAttribute('data-pagination')) {
      const paginationElement = document.querySelector(`.${el.getAttribute('data-pagination')}`);
      pagination = {
        el: paginationElement,
        bulletClass: 'pagination__item',
        bulletActiveClass: 'is-active',
    ...
    colsArray.forEach(col => {
      if (col.includes('base')) cols_base = parseFloat(col.slice(-3));
      if (col.includes('xl')) cols_xl = parseFloat(col.slice(-3));
      if (col.includes('lg')) cols_lg = parseFloat(col.slice(-3));
      if (col.includes('md')) cols_md = parseFloat(col.slice(-3));
      if (col.includes('sm')) cols_sm = parseFloat(col.slice(-3));
    });
    ...
    new Swiper(el, {
      ...
      breakpoints: {
        1199: { slidesPerView: parseFloat(cols_xl), spaceBetween: parseInt(gap) },
        991: { slidesPerView: parseFloat(cols_lg), spaceBetween: parseInt(gap) },
        767: { slidesPerView: parseFloat(cols_md), spaceBetween: parseInt(gap) },
        574: { slidesPerView: parseFloat(cols_sm), spaceBetween: parseInt(gap) },
      },
      lazy: {
        loadPrevNext: true,
      },
