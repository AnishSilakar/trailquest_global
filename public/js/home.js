...
jQuery(function ($) {
    ...
    $('.form-date-search').each(function () {
        ...
        var options = {
            ...
            minDate: today,
            opens: sarbatraCMSUrl.rtl ? 'right':'left',
            locale: {
                format: "YYYY-MM-DD",
                direction: sarbatraCMSUrl.rtl ? 'rtl':'ltr',
        ...
        check_in_out.daterangepicker(options,
            function (start, end, label) {
                check_in_input.val(start.format(sarbatraCMSUrl.date_format));
                check_in_render.html(start.format(sarbatraCMSUrl.date_format));
                check_out_input.val(end.format(sarbatraCMSUrl.date_format));
                check_out_render.html(end.format(sarbatraCMSUrl.date_format));
            });
    ...
    $('.date-picker').each(function () {
        var options = {
            ...
            opens: sarbatraCMSUrl.rtl ? 'right':'left',
            locale: {
                format: sarbatraCMSUrl.date_format,
                direction: sarbatraCMSUrl.rtl ? 'rtl':'ltr',
                firstDay:daterangepickerLocale.first_day_of_week
            }
        };
    ...
    $('.form-date-search-hotel').each(function () {
        ...
        check_in_out.daterangepicker(options).on('apply.daterangepicker',
            function (ev, picker) {
                ...
                check_in_input.val( picker.startDate.format(sarbatraCMSUrl.date_format) );
                check_in_render.html( picker.startDate.format(sarbatraCMSUrl.date_format) );
                check_out_input.val( picker.endDate.format(sarbatraCMSUrl.date_format) );
                check_out_render.html( picker.endDate.format(sarbatraCMSUrl.date_format) );
    ...
    $('.bravo-form-login [type=submit]').click(function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.two_factor){
                    return window.location.href = sarbatraCMSUrl.url + '/two-factor-challenge';
                }
                ...
                if(redirect.trim('/')){
                    window.location.href = sarbatraCMSUrl.url_root + form.find('input[name=redirect]').val();
                }else{
    ...
    $('.bravo-form-register [type=submit]').click(function (e) {
        ...
        $.ajax({
            'url':  sarbatraCMSUrl.routes.register,
            'data': {
                'email': form.find('input[name=email]').val(),
                'password': form.find('input[name=password]').val(),
    ...
    $(".smart-search .smart-search-location").each(function () {
        ...
        var options = {
            url: url ? url : sarbatraCMSUrl.url+'/location/search/searchForSelect2',
    ...
    $(document).on("click",".service-wishlist",function(){
        var $this = $(this);
        $.ajax({
            url:  sarbatraCMSUrl.url+'/user/wishlist',
            data: {
    ...
    $('.btn-submit-enquiry').click(function (e) {
        ...
        $.ajax({
            url:sarbatraCMSUrl.url+'/booking/addEnquiry',
    ...
    $('.review_upload_file').change(function () {
        ...
        for(var i = 0 ;i < me.get(0).files.length ; i++) {
            ...
            if(!me.showErr){
                $.ajax({
                    url: sarbatraCMSUrl.url + '/media/private/store',
                    data: d,
                    dataType: 'json',
                    type: 'post',
                    contentType: false,
                    ...
                    success: function (res) {
                        me.val('');
                        if (res.status === 0) {
                            sarbatraCMSUrlApp.showError(res);
                        }
                        if (res.data) {
                            var count = $(".review_upload_photo_list > .col-md-2").length;
                            if(count > 5){
                                sarbatraCMSUrlApp.showError('Maximum upload 6 pictures');
                            }else{
                    ...
                    error: function (e) {
                        sarbatraCMSUrlApp.showAjaxError(e);
                        me.val('');
                    }
                })
            }
...
jQuery(function($){
    ...
    if(sarbatraCMSUrl.pusher_api_key && sarbatraCMSUrl.pusher_cluster){
        var pusher = new Pusher(sarbatraCMSUrl.pusher_api_key, {
            encrypted: true,
            cluster: sarbatraCMSUrl.pusher_cluster
        });
    }
    ...
    $(document).on("click",".markAllAsRead",function(e) {
        ...
        e.preventDefault();
        $.ajax({
            url: sarbatraCMSUrl.markAllAsRead,
            method: "post",
            success:function (res) {
    ...
    if(sarbatraCMSUrl.isAdmin > 0 && sarbatraCMSUrl.pusher_api_key){
        var channel = pusher.subscribe('admin-channel');
        channel.bind('App\\Events\\PusherNotificationAdminEvent', callback);
    }

    if(sarbatraCMSUrl.currentUser > 0 && sarbatraCMSUrl.pusher_api_key){
        var channelPrivate = pusher.subscribe('user-channel-'+sarbatraCMSUrl.currentUser);
