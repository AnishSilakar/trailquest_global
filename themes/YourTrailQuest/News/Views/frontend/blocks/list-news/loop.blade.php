...
@if($style == 'style_5')
    <a href="{{$row->getDetailUrl()}}" class="blogCard -type-2 d-block bg-white rounded-4 shadow-4">
        ...
        <div class="px-20 py-20">
            ...
            <div class="text-light-1 text-15 lh-14 mt-10">{{ display_date($row->updated_at)}} | <i class="icon-user text-14"></i>  {{ $translation->author->getDisplayName() }}</div>
        </div>
    </a>
@elseif(!in_array($style,['style_2','style_4','style_6']))
    <div class="item-news">
        <a href="{{$row->getDetailUrl()}}" class="blogCard -type-1 d-block ">
            ...
            <div class="mt-20">
                <p class="blg-title">{!! clean($translation->title) !!}</p>
                <div class="text-light-1 text-15 lh-14 mt-5">{{ display_date($row->updated_at)}} | <i class="icon-user text-14"></i>  {{ $translation->author->getDisplayName() }}</div>
            </div>
        </a>
    </div>
@elseif($style == 'style_4')
    <a href="{{$row->getDetailUrl()}}" class="blogCard -type-3 ">
        ...
        <div class="blogCard__content px-50 pb-30 lg:px-20 pb-20">
            ...
            <div class="text-15 lh-14 text-white mt-10">{{ display_date($row->updated_at)}} | <i class="icon-user text-14"></i>  {{ $translation->author->getDisplayName() }}</div>
        </div>
    </a>
@elseif($style == 'style_6')
    <a href="{{$row->getDetailUrl()}}" class="row y-gap-20 items-center news-item">
        ...
        <div class="col">
            <div class="text-15 text-light-1">{{ display_date($row->updated_at)}} | <i class="icon-user text-14"></i> {{ $translation->author->getDisplayName() }}</div>
